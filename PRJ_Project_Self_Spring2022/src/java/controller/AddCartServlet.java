...
//        processRequest(request, response);
        ...
        List<Categories> list1 = ca.getAll();
        ...
        if (num.trim().length() == 0) {
        ...
        else if (Integer.parseInt(num) < 1 || Integer.parseInt(num) > p.getById(Integer.parseInt(id)).getStock()){
            request.setAttribute("err", "Over Stock!");
            request.setAttribute("num", num);
            request.getRequestDispatcher("detail").forward(request, response);
        }
        else {
            ...
            if (trans == null) {
                response.sendRedirect("detail?id="+id+"&cid="+cid);
            }
            else {
