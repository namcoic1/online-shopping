<%-- 
    Document   : addcatepro
    Created on : Mar 4, 2022, 10:08:58 PM
    Author     : Dell
--%>
...
<html>
    <head>
        ...
        <title>PM Shoes Shop | Update Product/Category/Shipper</title>
    ...
    <body>
        ...
        <c:if test="${requestScope.pro != null}">
            <h2 class="text mar">Update Products Form</h2>                    
            <div class="text mr-auto ml-auto form">
                <c:set var="ac" value="${requestScope.info}"></c:set>
                <h3 class="text" style="color: red">${requestScope.err}</h3>
                <form action="updateany" method="post">
                    <input type="hidden" name="n" value="pro"/>
                    <input type="hidden" name="id" value="${ac.id}"/>
                    Update Name: &nbsp;<input type="text" name="name" placeholder="enter name" value="${ac.name}" /><br/>
                    Update Category:<br/>
                    <c:forEach items="${requestScope.allcate}" var="cat">
                    <input type="radio" name="cid" value="${cat.id}" ${ac.cid == cat.id?"checked":""}>&nbsp;${cat.name}<br/>
                    </c:forEach>
                    Update Image: &nbsp;<input type="text" name="image" placeholder="enter absolute url" value="${ac.image}"/><br/>
                    Update Price: &nbsp;&nbsp;&nbsp;<input type="text" name="price" placeholder="..." value="${ac.price}"/><br/>
                    Update Stock: &nbsp;&nbsp;<input type="text" name="stock" placeholder="..." value="${ac.stock}"/><br/>
                    Update Detail: &nbsp;&nbsp;<input type="text" name="detail" placeholder="enter detail" value="${ac.detail}"/><br/><br/>
                    <input id="button" style="color: white" type="submit" value="Update"/>
                </form> 
            </div>
            <div class="text">
                <a type="button" style="margin-top: 2%; text-decoration: none" href="admmanage">&leftarrow;<span class="text-muted"> Back to Management</span></a>
            </div>
        </c:if>
            
        <c:if test="${requestScope.cate != null}">
            <h2 class="text mar">Update Category Form</h2>                    
            <div class="text mr-auto ml-auto form">
                <c:set var="ac" value="${requestScope.info}"></c:set>
                <h3 class="text" style="color: red">${requestScope.err}</h3>
                <form action="updateany" method="post">
                    <input type="hidden" name="n" value="cate"/>
                    <input type="hidden" name="id" value="${ac.id}"/>
                    Update Name: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="name" placeholder="enter name" value="${ac.name}" /><br/>
                    Update Image:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="image" placeholder="enter absolute url" value="${ac.image}"/><br/>
                    Update Describe: &nbsp;<input type="text" name="detail" placeholder="enter detail" value="${ac.des}"/><br/><br/>
                    <input id="button" style="color: white" type="submit" value="Update"/>
                </form> 
            </div>
            <div class="text">
        ...
        <c:if test="${requestScope.ship != null}">
            <h2 class="text mar">Update Shipper Form</h2>                    
            <div class="text mr-auto ml-auto form">
                <c:set var="ac" value="${requestScope.info}"></c:set>
                <h3 class="text" style="color: red">${requestScope.err}</h3>
                <form action="updateany" method="post">
                    <input type="hidden" name="n" value="ship"/>
                    <input type="hidden" name="id" value="${ac.id}"/>
                    Update Name: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="name" placeholder="enter name" value="${ac.name}" /><br/>
                    Update Phone:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="phone" placeholder="enter absolute url" value="${ac.phone}"/><br/>
                    Update Status: &nbsp;<input type="radio" name="sta" value="1" ${ac.status eq 'true'? "checked":""}/> On <input type="radio" name="sta" value="0" ${ac.status eq 'false'? "checked":""}/> Off<br/><br/>
                    <input id="button" style="color: white" type="submit" value="Update"/>
                </form> 
            </div>
            <div class="text">
